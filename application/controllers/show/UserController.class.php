...
class UserController extends BaseController
{
    ...
    public function commonAction(){
        ...
        if($request=='POST'){
            ...
            if ($_GET['type']==1){
                ...
                $check = new Model('dljz');
                $row = $check->select("select *from sl_dljz WHERE id={$_GET['id']}")[0];
                $list['type'] = '代理记账';
                $list['sort_id'] = 83;
                $list['fwxq'] = $row['leixing'];
                $list['fwjg'] = $row['jiage'];
            }else{
                $list['type'] = '企业变更';
                $list['sort_id'] = 84;
                $check = new Model('taocan');
                $row = $check->select("select *from sl_taocan WHERE id={$_GET['id']}")[0];
                $list['fwxq'] = $row['leixing']."-".$row['fwxq'];
                $list['fwjg'] = $row['jiage']-$row['youhui'];
            }
            $list['uid'] = $uid;
            $list['clzt'] = '处理中';
            $list['fws'] = '店来店往自营';
    ...
    public function rechargeAction(){
        ...
        if($request=="POST"){
            ...
            $no = date('Ymd',time());
            $row = $model->select("select *from sl_count WHERE `day`={$no} and `type`='充值'  limit 0,1");
        ...
        $uid = $_SESSION['user_id'];
        $account = new Model('recharge');
        $list = $account->select("select *from sl_acount WHERE uid = {$uid} limit 0,1")[0];
    ...
    public function cashAction(){
        ...
        $uid = $_SESSION['user_id'];
        $request = $_SERVER['REQUEST_METHOD'];
        $balance = $model->select("select paywords,property,tel from sl_member WHERE id = $uid");
        $lists = $model->select("select *from sl_acount WHERE uid = {$uid}");
        if ($request=='POST'){
            if (empty($balance[0]['paywords'])){
                return $this->jump("?c=user&a=paywords","请先设置支付密码",3);
            }
            if ($_POST['yzm']!=$_SESSION[$balance[0]['tel']]||!$_SESSION[$balance[0]['tel']]){
                return $this->jump("?c=user&a=cash","验证码错误",3);
            }
            $pay_words = md5($_POST['paywords']);

            if ($balance[0]['paywords']!=$pay_words){
                return $this->jump("?c=user&a=cash","支付密码错误",3);
            }
            //生成流水号
            $model->start_T();
            try{
                $no = date('Ymd',time());
                $row = $model->select("select *from sl_count WHERE `day`={$no} and `type`='提现' limit 0,1");
                $number = new Model("count");
                if (!$row){
                    $arr['day'] = $no;
                    $arr['count'] = 1;
                ...
                $data['status'] = '处理中';
                $data['uid'] = $uid;
                $data['bank'] = $lists[0]['khyh'];
                $data['account'] = $lists[0]['yhkh'];
                ...
                $log['mark'] = '支出';
                $log['intro'] = '无';
                $log['bank'] = $lists[0]['khyh'];
                $log['account'] = $lists[0]['yhzh'];
                $log['balance'] = $balance[0]['property']-$_POST['price'];
                $insert = new Model('log');
                if (!$insert->insert($log)){
                    throw new Exception("");
                }
                //减少资产
                if (!$model->xg(['property'=>$log['balance']],"id={$uid}")){
                    throw new Exception("");
                }
                $model->comit_T();
                $this->jump("?c=user&a=log","提现成功",3);
            }catch (Exception $e){
                $model->roll_T();
                $this->jump("?c=user&a=cash","提现失败",3);
            }
        }
        $count = $model->select("select count(*) as total from sl_acount WHERE uid=$uid");
        include CUR_VIEW_PATH . "Suser" . DS ."cash.html";
    }

    public function bindAction(){
        $model = new Model('acount');
        $request = $_SERVER['REQUEST_METHOD'];
        if ($request=='POST'){
            $data = $_POST;
            $data['uid'] = $_SESSION['user_id'];
            if ($data['yzm']!=$_SESSION[$data['yldh']]){
                return $this->jump("?c=user&a=bind","验证码错误",3);
            }
            unset($data['yzm']);
            $re = $model->insert($data);
            if ($re){
                return $this->jump("?c=user&a=cash","添加成功",3);
            }else{
                return $this->jump("?c=user&a=bind","绑定失败",3);
            }
        }
        include CUR_VIEW_PATH . "Suser" . DS ."bind.html";
    }
...
//        var_dump("select *from sl_record $where ORDER BY dtime DESC ");die;
        $list = $model->select("select *from sl_record $where ORDER BY dtime DESC ");
    ...
    public function frozenAction(){
        ...
        $list = $model->select("select *from sl_frozen $where ORDER BY dtime DESC ");
    ...
    public function paywordsAction(){
        ...
        $uid = $_SESSION['user_id'];
        //验证是否为首次设置密码
        $paywords = $model->select("select paywords,tel from sl_member WHERE id={$uid}")[0];
        if ($request=='POST'){
            $data = $_POST;
            if ($data['yzm']!=$_SESSION[$paywords['tel']]||!$_SESSION[$paywords['tel']]){
                $this->jump('index.php?p=show&c=user&a=paywords','验证码错误',3);
            }
            unset($data['yzm']);
